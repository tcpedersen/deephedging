...
folder_name = r"results\experiment-9"
...
for num in range(number_of_tests):
    ...
    #     init_instruments, init_numeraire, book, spread)
    ...
    for degree in tf.range(2, 10):
        warmup_driver.add_testcase(
            name=f"polynomial {degree}",
            model=gradient_models.SequencePolynomial(degree),
...
file_name = os.path.join(folder_name, fr"dimension-{dimension}.txt") # TODO remove extra
